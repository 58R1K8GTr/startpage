...
- **9 Themes** — Light, Dark, AMOLED Black, Nord, Newspaper, Coffee, Root (Hacker), Neon (Cyberpunk), and Matugen (Dynamic Material You / Pywal wallpaper colors).
...
| `:matugen` / `:material` | Dynamic Matugen (Material You / Pywal wallpaper colors) |
| `:matugen manual` / `:matugen off` | Disable 2s background polling (Manual mode — protects SSD) |
| `:matugen auto` / `:matugen 2s` | Enable 2s background polling (Auto mode — checks every 2s) |
...
### Matugen & Pywal Dynamic Themes — `:matugen` / `:material`

Integrates dynamic wallpaper-based colors generated by **Matugen** or **Pywal** (`~/.cache/wal/colors.json` or `~/.cache/matugen`).

- **SSD Protection & Refresh Modes:**
  - **`:matugen manual` (or `:matugen off`):** Disables background polling completely. Zero background disk reads to protect your SSD from wear. Theme updates only when you manually select a file or run `:matugen`.
  - **`:matugen auto` (or `:matugen 2s`):** Enables 2-second background polling. Monitors color file updates and applies new wallpaper colors live on screen.
- **Pywalfox Firefox Extension Integration:** Automatically detects Pywalfox CSS variables injected by the browser extension.
- **File System Access API (Chromium / Brave / Edge / Opera):** Selecting `~/.cache/wal/colors.json` via file picker stores an IndexedDB handle so live watching works across wallpaper changes.
- **Symlink Setup (Universal for Firefox / All Browsers):**
  Create a symlink inside the startpage project folder to enable live relative file polling:
  ```bash
  ln -s ~/.cache/wal/colors.json ./colors.json
  ```
